...
class ArticleController extends CommonController{
    ...
    public function checkListing(){
        ...
        $article_model = D('Article');
        $p = I('get.p') ? I('get.p') : 1;
        $article_data = $article_model->alias('a')->field('article_id,a.cate_id,title,cate_name,author,newstime')->join('left join __CATEGORY__ c on a.cate_id = c.cate_id')->where($map)->order('article_id desc')->page($p.',20')->select();
        $this->assign('article_data',$article_data);
        //分页数据
