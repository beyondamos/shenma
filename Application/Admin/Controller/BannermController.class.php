...
class BannermController extends CommonController
{
	/**
	 * Banner列表
	...
	public function add()
	{
		if (IS_POST) {
			...
			if ($banner_model->create()) {
				if ($banner_model->add()) {
					$this->success('Banner添加成功', U('Bannerm/index', array('classify' => I('post.classify'))), 1);
				} else {
	...
	public function delete()
	{
        ...
        if($banner_model->delete($id)){
            $this->success('Banner删除成功', U('Bannerm/index', array('classify' => $classify)), 1);
        }else{
