			...
			<table class="table table-hover boxtable">
				<thead>
					<tr>
					   ...
					   <th class="col-md-1 text-vm">是否显示</th>
					   <th class="col-md-1 text-vm">是否新页面</th>
				...
				<tbody>
					...
					<?php if(is_array($nav_data)): $i = 0; $__LIST__ = $nav_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td class="text-vm"><?php echo ($vo["nav_name"]); ?></td>
						<td class="text-vm"><?php echo ($vo["nav_url"]); ?></td>
						<td class="text-vm"><?php if($vo['is_show'] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
						<td class="text-vm"><?php if($vo['is_blank'] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
						<td class="text-vm"><?php echo ($vo["sort"]); ?></td>
						<td class="text-vm">
							<a href="<?php echo U('Nav/edit',array('nav_id' => $vo['nav_id']));?>">编辑</a>
							<a href="<?php echo U('Nav/delete', array('nav_id' => $vo['nav_id']));?>">删除</a>
						</td>
