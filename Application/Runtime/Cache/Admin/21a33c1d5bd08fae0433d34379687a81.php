...
<div class="cont">
	<div class="contmain">
		...
		<div class="boxi">
			...
			<!-- 表格顶部搜索区 -->
			<div class="boxoper">
				<a href="<?php echo U('Videoc/add');?>">添加分类</a>
				<!---->
			...
			<table class="table table-hover boxtable">
				<thead>
					<tr>
					   <th class="col-md-2 text-vm">分类名称</th>
					   <th class="col-md-1 text-vm text-center">操作</th>
					</tr>
				</thead>
				<tbody>

					<?php if(is_array($videoc_data)): $i = 0; $__LIST__ = $videoc_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td class="text-vm"><?php echo str_repeat('---------', $vo['level']); echo ($vo["name"]); ?></td>
						<td class="text-vm">
							<a href="<?php echo U('Videoc/edit',array('id' => $vo['id']));?>">编辑</a>
							<a href="<?php echo U('Videoc/delete', array('id' => $vo['id']));?>">删除</a>
						</td>
