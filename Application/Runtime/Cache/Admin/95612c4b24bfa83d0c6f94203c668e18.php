	...
	<div class="contmain">
		<div class="boxi">
			...
			<form action="<?php echo U('Category/add');?>" method="post">
				<div class="boxin">
					<span>分类名称</span><input type="text" name="cate_name" class="form-control">
				</div>
				<div class="boxin">
					<span>上级分类</span>
					<select name="parent_id" class="form-control">
						<option value="0">请选择分类</option>
						<?php if(is_array($categroy_data)): $i = 0; $__LIST__ = $categroy_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['cate_id']); ?>"><?php echo str_repeat('----',$vo['level']); echo ($vo['cate_name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</div>
				<div class="boxin">
					<span>栏目标题</span><input type="text" class="form-control" name="cate_title">
				</div>
				<div class="boxin">
					<span>关键词</span><input type="text" class="form-control" name="cate_keywords">
				</div>
				<div class="boxtext">
					<span>分类描述</span><textarea rows="6" class="form-control" name="description"></textarea>
				</div>
