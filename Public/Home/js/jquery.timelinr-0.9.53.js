...
jQuery.fn.timelinr = function(options){
	...
	settings = jQuery.extend({
		...
		datesDiv: 					'#datesb',			// value: any HTML tag or #id, default to #dates
			...
			if(settings.orientation == 'horizontal') {
				...
				if(currentPositionIssues <= -(widthIssue*howManyIssues-(widthIssue))) {
					...
					if (!$(settings.issuesDiv).is(':animated')) {
						...
						$(settings.datesDiv+' a.'+settings.datesSelectedClass).removeClass(settings.datesSelectedClass).parent().next().children('a').addClass(settings.datesSelectedClass);
					}
				}
			} else if(settings.orientation == 'vertical') {
			...
			if(settings.orientation == 'horizontal') {
				...
				if(currentPositionIssues >= 0) {
					...
					if (!$(settings.issuesDiv).is(':animated')) {
						...
						$(settings.datesDiv).animate({'marginLeft':currentIssueDate},{queue:false, duration:'settings.datesSpeed'});
						$(settings.datesDiv+' a.'+settings.datesSelectedClass).removeClass(settings.datesSelectedClass).parent().prev().children('a').addClass(settings.datesSelectedClass);
					}
				}
			} else if(settings.orientation == 'vertical') {
				var currentPositionIssues = parseInt($(settings.issuesDiv).css('marginTop').substring(0,$(settings.issuesDiv).css('marginTop').indexOf('px')));
